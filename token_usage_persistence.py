...
class TokenUsagePersistence:
    ...
    def save_usage(self, token_usage: TokenUsage, session_id: str = None) -> bool:
        ...
        try:
            ...
            session_data = {
                "session_id": session_id or f"session_{token_usage.total_input + token_usage.total_output}",
                "input_tokens": token_usage.total_input,
                "output_tokens": token_usage.total_output,
                "total_tokens": token_usage.total_input + token_usage.total_output,
            ...
            if session_id:
                ...
                # Find latest session and append
    ...
    def load_usage(self, session_id: str = None) -> Optional[TokenUsage]:
        ...
        try:
            ...
            if session_id and session_id in data.get("sessions", {}):
                ...
                return TokenUsage(
                    total_input=session_data["input_tokens"],
                    total_output=session_data["output_tokens"]
                )
            elif "sessions" in data and data["sessions"]:
                # Return latest session
                latest_session = data["sessions"][-1]
                return TokenUsage(
                    total_input=latest_session["input_tokens"],
                    total_output=latest_session["output_tokens"]
                )
            else:
                return None
    ...
    def get_usage_summary(self) -> Dict[str, Any]:
        ...
        total_input = sum(session["input_tokens"] for session in data.get("sessions", []))
        total_output = sum(session["output_tokens"] for session in data.get("sessions", []))
        total_cost = sum(session.get("estimated_cost", 0) for session in data.get("sessions", []))
        ...
        return {
            ...
            "recent_sessions": data.get("sessions", [])[-5:]  # Last 5 sessions
        }
    ...
    def clear_old_sessions(self, keep_last: int = 50) -> bool:
        ...
        try:
            data = self.load_all_usage()
            if "sessions" in data and len(data["sessions"]) > keep_last:
                data["sessions"] = data["sessions"][-keep_last:]
